...
def done():
    ...
    """
    	go_to_point(goal):
    		Function used to action server in order to allow the robot to reach the correct target position
    		Parameter: goal contain the position and orientation of the goal
...
def go_to_point(goal):
    ...
    while True:
        ...
        if state_ == 0:
            ...
            break
    ##when target is reached the result is set to succedded
    server.set_succeeded(result)
    return True

"""
   The main function of the script:
 	Create a publisher on topic /cmd_vel to publish the velocity of the robot
	Create a subscriber of topic /odom to receive the position of the robot
"""
def main():
    global pub_, server
